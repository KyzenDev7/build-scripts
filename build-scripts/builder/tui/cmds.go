...
func runChecksCmd(checks []Check) tea.Cmd {
	return func() tea.Msg {
		...
		// Launch the appropriate check goroutine for each requested check.
		// This drives execution from the provided `checks` slice and keeps
		// the expected result count consistent with launched goroutines.
		for _, c := range checks {
			...
			case "Storage space (>20 GB free)":
				go checkStorage(results)
			case "OS compatibility (Debian/Ubuntu)":
				go checkOS(results)
			case "Required tools (debootstrap, squashfs)":
				go checkTools(results)
			default:
				// Unknown check — report failure for that check
				go func(name string) { results <- CheckResultMsg{Name: name, Passed: false, Detail: "unknown check"} }(c.Name)
			}
		}

		// Collect results and send each as a message to the UI so rows update live.
		passed := true
		for i := 0; i < len(checks); i++ {
			r := <-results
			// Send individual check result to the Bubble Tea program for UI updates.
			if p != nil {
				p.Send(r)
			}
			if !r.Passed {
...
func statfs(path string, stat *syscallStatfs) error {
	...
	// On non-Linux platforms, return an error so callers can handle it.
